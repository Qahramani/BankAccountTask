...
namespace Events
{
    ...
    internal class BankAccount
    {
        ...
        private void OverdrawnEventMethod(object sender, EventArgs e)
        {
            ...
            {
            MainAccount mainAcc = (MainAccount)sender;
            MessageBox.Show("Balansinda pul yoxdur,  " + args.SubAmount + " " + (mainAcc.Balance + mainAcc.SavingAccount.Balance));
            }
            else if(sender is SavingAccount) 
            {
                SavingAccount savingAcc = (SavingAccount)sender;
                MessageBox.Show($"Balansinda pul yoxdu, Balans : {args.SubAmount}");
            }
